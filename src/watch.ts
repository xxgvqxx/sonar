  ...
  for (const w of matched) {
    try {
      ...
      try {
        ...
        /* no pane / busy — the posted message still lands */
      }
      try {
        appendLog(opts.linkId, WATCH_LABEL, `watch fired for **${w.label}** ${scope}: ${opts.detail}${woke ? ' · woke pane' : ''}`);
      } catch {
        /* log is best-effort */
      }
      fired.push({ id: w.id, label: w.label, event: w.event, woke });
    } catch {
